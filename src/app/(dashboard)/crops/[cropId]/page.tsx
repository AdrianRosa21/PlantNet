  ...
  return (
    <div className="relative min-h-[calc(100vh-80px)] w-full overflow-hidden bg-background">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[0%] -left-[10%] w-[60%] h-[60%] rounded-full bg-primary/20 blur-[130px] mix-blend-multiply animate-pulse" />
        <div className="absolute top-[30%] -right-[15%] w-[70%] h-[70%] rounded-full bg-secondary/30 blur-[150px] mix-blend-multiply opacity-60" />
        <div className="absolute bottom-[0%] left-[20%] w-[50%] h-[50%] rounded-full bg-muted/40 blur-[120px] mix-blend-multiply opacity-50" />
        <div className="absolute inset-0 bg-background/40 backdrop-blur-[50px] z-0" />
      </div>

      <div className="relative z-10 w-full max-w-3xl mx-auto space-y-6 pt-5 pb-20 px-4 sm:px-6">

        {/* HEADER PRINCIPAL */}
        <div className="flex items-center gap-4 animate-in slide-in-from-top-4 duration-700 ease-out">
          <Button variant="ghost" size="icon" onClick={() => router.back()} className="rounded-full bg-background/50 backdrop-blur-md shadow-sm border border-primary/20 hover:-translate-y-1 hover:shadow-md transition-all text-muted-foreground hover:text-primary flex-shrink-0">
            <ArrowLeft className="w-6 h-6" />
          </Button>
          <div className="flex-1 min-w-0 flex items-center gap-2">
            <div className="flex-1 truncate">
              <h1 className="text-3xl sm:text-4xl font-black tracking-tight truncate text-foreground pb-1">{crop.name}</h1>
              <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">{crop.type}</p>
            </div>
            <CropTour />
          </div>
        </div>
        ...
        {/* CALENDARIO BURBUJAS */}
        {weekDays.length > 0 && (
          <div id="tour-calendar" className="flex items-center justify-between bg-background/50 backdrop-blur-xl rounded-[2rem] p-4 shadow-[0_8px_32px_rgba(0,0,0,0.04)] border border-foreground/15 animate-in slide-in-from-bottom-4 duration-700 delay-100 ease-out">
            {weekDays.map((day, i) => {
              ...
              return (
                <button 
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center w-[13%] sm:w-[12%] aspect-[3/4] rounded-2xl transition-all duration-300 border border-transparent",
                    isSelected 
                      ? "bg-primary text-primary-foreground shadow-xl shadow-primary/30 scale-105 -translate-y-1" 
                      : "text-muted-foreground hover:bg-muted/50 hover:shadow-sm"
                  )}
                >
                  <span className={cn("text-[10px] sm:text-xs font-bold uppercase", isSelected ? "text-primary-foreground/90" : "text-muted-foreground/80")}>{day.dayName}</span>
                  <span className={cn("text-lg sm:text-xl font-black mt-0.5", isSelected ? "text-primary-foreground drop-shadow-sm" : "text-foreground/90")}>{day.dayNum}</span>
                  {isTodayNode && <div className={cn("w-1.5 h-1.5 rounded-full mt-1.5 shadow-sm", isSelected ? "bg-primary-foreground animate-pulse" : "bg-primary")} />}
                </button>
              )
            })}
          </div>
        ...
        {!isToday && (
          <Alert className="bg-secondary/10 backdrop-blur-md border-secondary/20 text-secondary-foreground rounded-3xl animate-in zoom-in duration-500 shadow-inner">
            <AlertTitle className="text-[13px] font-black flex items-center gap-2 uppercase tracking-widest text-secondary-foreground">
              <Calendar className="w-4 h-4" />
              Modo Historial
            </AlertTitle>
            <AlertDescription className="text-sm mt-1.5 font-medium text-secondary-foreground/90">
              Estás visualizando la bitácora del <strong className="font-black text-secondary bg-secondary/20 px-1 py-0.5 rounded">{selectedDateStr}</strong>. Los registros pasados son de lectura estricta.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-bottom-8 duration-700 delay-200 ease-out">
          ...
          <Card id="tour-water" className="rounded-[2rem] border border-foreground/15 shadow-[0_8px_32px_rgba(0,0,0,0.05)] overflow-hidden bg-background/60 backdrop-blur-xl relative group hover:bg-background/80 transition-all">
            <CardContent className="p-5 flex flex-col items-center justify-center text-center space-y-3 relative z-10">
              <div className="relative w-20 h-24">
                <svg viewBox="0 0 30 42" className="w-full h-full drop-shadow-xl transition-transform duration-700 group-hover:scale-105 group-hover:-translate-y-1">
                  <path d="M15 2 C15 2 27 18 27 28 A12 12 0 0 1 3 28 C3 18 15 2 15 2 Z" fill="var(--background)" className="stroke-primary/20 opacity-80" strokeWidth="1" />
              ...
              <div className="space-y-0.5">
                <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">{isToday ? "Progreso" : "Registro"}</p>
          ...
          <Card className="rounded-[2rem] border border-foreground/15 shadow-[0_8px_32px_rgba(0,0,0,0.05)] bg-background/60 backdrop-blur-xl relative group hover:bg-background/80 transition-all">
            <CardContent className="p-5 flex flex-col items-center justify-center text-center space-y-4 h-full relative z-10">
              <div className="relative">
                ...
                <div className="w-16 h-16 bg-orange-50/80 rounded-2xl flex items-center justify-center border border-primary/10 shadow-inner relative z-10 transform group-hover:-translate-y-1 transition-all duration-500">
                  <Thermometer className="w-8 h-8 text-orange-500 drop-shadow-sm" />
                </div>
              </div>
              <div className="space-y-0.5">
                <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Temperatura</p>
                <p className="text-2xl font-black text-foreground tracking-tighter">{crop.idealTemperature ?? 24}°C <span className="text-sm text-foreground/60 font-bold">Ideal</span></p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card id="tour-irrigation" className="rounded-[2rem] shadow-[0_8px_32px_rgba(0,0,0,0.03)] border border-foreground/15 bg-background/50 backdrop-blur-xl animate-in slide-in-from-bottom-8 duration-700 delay-300 ease-out">
          <CardContent className="p-5 flex flex-col sm:flex-row items-center gap-4">
            <div className="flex-1 flex items-center gap-4 w-full">
              <div className="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center shrink-0 border border-white/50 shadow-inner relative overflow-hidden">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  <Badge variant="outline" className={cn("text-[9px] uppercase tracking-widest font-black border-transparent shadow-sm", irrigationData.status === "Riego Óptimo" ? "bg-primary/20 text-primary" : irrigationData.status === "Sobre-regado" ? "bg-red-50 text-red-700" : "bg-blue-50 text-blue-700")}>
                    {irrigationData.status}
                  </Badge>
                </div>
                <p className="text-sm font-medium text-foreground/80 leading-tight">"{irrigationData.message}"</p>
              </div>
            </div>
            ...
            <Button 
              ...
              className={cn(
                ...
                isToday 
                  ? "bg-blue-500 hover:bg-blue-600 text-white shadow-blue-500/25 hover:shadow-blue-500/40 hover:-translate-y-1 active:scale-95" 
        ...
        {/* TAREAS ACRÍLICAS */}
        <Card id="tour-tasks" className="rounded-[2rem] shadow-[0_8px_32px_rgba(0,0,0,0.03)] border border-foreground/15 bg-background/50 backdrop-blur-xl animate-in slide-in-from-bottom-8 duration-700 delay-500 ease-out mt-8">
          <CardHeader className="pb-3 px-6 pt-6">
            <CardTitle className="text-xl font-black flex items-center justify-between text-foreground">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-secondary/20 rounded-xl flex items-center justify-center border border-primary/10 shadow-inner">
                  <StickyNote className="w-5 h-5 text-secondary drop-shadow-sm" />
                </div>
                Bitácora Táctica
              </div>
              {!isToday && <Badge variant="secondary" className="text-[10px] font-black uppercase tracking-widest bg-muted text-muted-foreground">Archivo</Badge>}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 px-6 pb-6">
            {isToday && (
              <div className="flex gap-2 relative group mt-1">
                <Input 
                  ...
                  className="rounded-2xl h-14 bg-background/80 border-primary/20 pr-16 text-[15px] font-medium placeholder:text-muted-foreground focus-visible:ring-primary focus-visible:border-primary shadow-sm transition-all group-hover:bg-background text-foreground"
                />
                <Button onClick={handleAddTask} disabled={!newNoteContent.trim()} size="icon" className="rounded-xl h-10 w-10 absolute right-2 top-2 bg-primary hover:bg-primary/90 border-none shadow-md hover:-translate-y-0.5 transition-all text-primary-foreground active:scale-95">
            ...
            <div className="space-y-2 mt-4">
              {tasks && tasks.length > 0 ? (
                tasks.map((task) => (
                  <div key={task.id} className={cn("flex items-center justify-between p-4 rounded-2xl border transition-all duration-300", task.completed ? "bg-background/40 border-transparent shadow-none" : "bg-background/80 border-primary/10 shadow-[0_4px_16px_rgba(0,0,0,0.02)] hover:-translate-y-0.5 hover:shadow-sm")}>
                    <div className="flex items-center gap-4 overflow-hidden">
                      <button 
                        onClick={() => isToday && handleToggleTask(task.id, task.completed)}
                        disabled={!isToday}
                        className={cn("flex-shrink-0 w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all duration-300", task.completed ? "bg-primary border-primary text-primary-foreground shadow-inner" : "border-muted-foreground bg-muted/50 hover:border-primary")}
                      >
                        {task.completed && <Check className="w-4 h-4 stroke-[3]" />}
                      </button>
                      <span className={cn("text-[15px] truncate transition-colors duration-300", task.completed ? "line-through text-muted-foreground font-medium" : "text-foreground font-bold")}>{task.content}</span>
                    </div>
                    {isToday && (
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteTask(task.id)} className="h-8 w-8 text-muted-foreground hover:text-red-500 hover:bg-red-50 shrink-0 rounded-full transition-colors">
                ...
                <div className="flex flex-col items-center py-8 opacity-70">
                  <ListChecks className="w-10 h-10 text-muted-foreground/60 mb-2" />
                  <span className="text-sm font-semibold text-muted-foreground">{isToday ? "Diario en blanco. ¡Registra una acción!" : "No hubo reportes tácticos este día."}</span>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <div className="flex gap-3 mt-8 animate-in slide-in-from-bottom-4 duration-700 delay-1000">
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="secondary" className="flex-1 gap-2 h-14 rounded-2xl bg-background/70 backdrop-blur-md border border-primary/20 shadow-[0_4px_16px_rgba(0,0,0,0.03)] text-foreground font-bold hover:bg-background hover:text-primary transition-all">
                <Pencil className="w-4 h-4" /> Modificar Flora
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px] overflow-hidden max-h-[90vh] bg-background/90 backdrop-blur-3xl border border-primary/20 shadow-2xl rounded-3xl">
              <DialogHeader className="pt-4 pb-2">
                <div className="mx-auto w-16 h-16 bg-muted rounded-2xl flex items-center justify-center mb-3 shadow-inner border border-primary/10 relative">
                  <Pencil className="w-7 h-7 text-primary drop-shadow-sm relative z-10" />
                  <div className="absolute inset-0 border border-primary/20 rounded-2xl animate-ping opacity-50 duration-3000"></div>
                </div>
                <DialogTitle className="text-center font-black text-2xl tracking-tight text-foreground">ADN del Cultivo</DialogTitle>
              </DialogHeader>
              <div className="grid gap-5 py-4 px-2">
                <div className="grid gap-2.5">
                  <Label className="text-[11px] font-black uppercase tracking-widest text-foreground/70">Genotipo Gráfico</Label>
                  <div className="grid grid-cols-6 gap-2">
                    {CROP_ICONS.map((item) => (
                      <button key={item.name} onClick={() => setEditForm({ ...editForm, icon: item.name })} className={cn("flex items-center justify-center h-12 rounded-xl border-2 transition-all", editForm.icon === item.name ? "border-primary bg-muted text-primary shadow-sm" : "border-transparent bg-background hover:bg-muted text-muted-foreground")}>
                ...
                <div className="grid gap-2.5">
                  <Label htmlFor="edit-name" className="text-[11px] font-black uppercase tracking-widest text-foreground/70">Alías de Planta</Label>
                  <Input id="edit-name" value={editForm.name} onChange={(e) => setEditForm({...editForm, name: e.target.value})} className="h-12 bg-background rounded-xl border-primary/20 shadow-sm text-[15px] font-medium focus-visible:ring-primary text-foreground" />
                </div>
                <div className="grid gap-2.5 relative" ref={dropdownRef}>
                  <Label className="text-[11px] font-black uppercase tracking-widest text-foreground/70">Familia Botánica</Label>
                  <Input value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setIsDropdownOpen(true); }} placeholder="Ej: Arbusto, Suculenta..." className="h-12 bg-background rounded-xl border-primary/20 shadow-sm text-[15px] font-medium focus-visible:ring-primary text-foreground" />
                  {isDropdownOpen && (
                    <div className="absolute top-[70px] left-0 right-0 z-50 bg-background/95 backdrop-blur-xl border border-primary/10 rounded-xl shadow-xl max-h-48 overflow-auto py-1">
                      {filteredPlantTypes.map(type => (
                        <button key={type} className="w-full text-left px-5 py-3 hover:bg-muted hover:text-foreground text-sm font-medium transition-colors text-foreground" onClick={() => { setEditForm({...editForm, type}); setSearchQuery(type); setIsDropdownOpen(false); }}>{type}</button>
                      ))}
                    </div>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2.5">
                    <Label className="text-[11px] font-black uppercase tracking-widest text-foreground/70">Agua Diaria</Label>
                    <Input type="number" value={editForm.dailyIrrigationGoal} onChange={(e) => setEditForm({...editForm, dailyIrrigationGoal: e.target.value})} className="h-12 bg-background rounded-xl shadow-sm text-center font-bold text-lg text-blue-600 focus-visible:ring-blue-500 border-primary/20" />
                  </div>
                  <div className="grid gap-2.5">
                    <Label className="text-[11px] font-black uppercase tracking-widest text-foreground/70">Temp IDEAL (°C)</Label>
                    <Input type="number" value={editForm.idealTemperature} onChange={(e) => setEditForm({...editForm, idealTemperature: e.target.value})} className="h-12 bg-background rounded-xl shadow-sm text-center font-bold text-lg text-orange-500 focus-visible:ring-orange-500 border-primary/20" />
                  </div>
                </div>
              </div>
              <DialogFooter className="pb-2">
                <Button onClick={handleUpdateCrop} className="w-full h-14 rounded-xl text-[15px] font-black bg-primary hover:bg-primary/90 text-primary-foreground shadow-xl shadow-primary/20 active:scale-95 transition-transform border-none">Aplicar Mutación</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
